  ...
  <div class="draftDetailCon">
    <van-form @failed="onFailed" @submit="onSubmit" error-message-align="right">
      ...
      <div class="listCon marBottom30">
        <template v-for="(item, index) in state.formData.build_contents" :key="index">
          <div class="marBottom30 itemCon">
            ...
            <!-- <div class="endBtn">
              <van-button class="littleBtn" round :class="state.ZLBFitForOld ? 'littleBtnOld' : ''" block type="primary" @click="deleteCurrent(index)">删除</van-button>
            </div> -->
          </div>
        </template>
      </div>
      <!-- <div class="addbtnCon" @click="addMore">
      ...
      <div class="fixBottom fixPaddingBottom">
        <van-button class="saveBtn" :class="state.ZLBFitForOld ? 'btnTextOld' : ''" block @click="saveAction">保存</van-button>
        <van-button class="submitBtn" :class="state.ZLBFitForOld ? 'btnTextOld' : ''" block type="primary" native-type="submit">下一步</van-button>
      </div>
    </van-form>
    ...
    <van-calendar :lazy-render="false" v-model:show="state.calenderShow" type="range" @confirm="onCalenderConfirm" />
