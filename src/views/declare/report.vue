<template>
  <div class="projectReservePublishCon">
    <div class="headTitle">建设项目申报书</div>
    <van-form @failed="onFailed" @submit="onSubmit">
      <div class="topCon">
        ...
        <van-field
          required
          input-align="right"
          v-model="state.formData.village_start_audit_at"
          disabled
          name="village_start_audit_at"
          label="申报日期"
          :class="state.ZLBFitForOld ? 'fieldOld' : ''"
          :rules="state.validator.village_start_audit_at"
          placeholder="请选择申报日期"
        />
      </div>
    ...
    <van-calendar v-model:show="state.calenderShow" type="range" :lazy-render="false" :min-date="state.minDate" :max-date="state.maxDate" @confirm="onCalenderConfirm" />
    <van-calendar :lazy-render="false" :min-date="state.minDate" :max-date="state.maxDate" v-model:show="state.singleCalenderShow" @confirm="onSingleConfirm" />
  </div>
</template>
<script>
  ...
  setup(props, { emit }) {
    ...
    const state = reactive({
      ...
      validator: {
        ...
        // build_at: [{ trigger: 'onBlur', message: '请选择', required: true }],
        area_detail: [{ trigger: 'onBlur', message: '请输入', required: false }],
        village_start_audit_at: [{ trigger: 'onBlur', message: '请选择', required: true }],
        unit_charge_mobile: [
          {
            required: true,
            trigger: 'onBlur',
    ...
    const normalSet = (res) => {
      ...
      if (state.formData.village_start_audit_at) {
        state.formData.village_start_audit_at = dayjs(state.formData.village_start_audit_at).format('YYYY-MM-DD')
      } else {
        state.formData.village_start_audit_at = dayjs().format('YYYY-MM-DD')
      }
        ...
        Request.projectReserveDetail({
          ...
          .then((res2) => {
            if (res2 && res2.code == 0) {
              ...
              state.formData = shorter
              state.formData.area_ids = res.data.info.area_ids
              if (shorter.build_start_at) {
                // state.build_at = `${dayjs(shorter.build_start_at).format('YYYY-MM')}-${dayjs(shorter.build_end_at).format('YYYY-MM')}`
              }
              if (state.formData.declare_at) {
                state.formData.village_start_audit_at = dayjs(state.formData.declare_at).format('YYYY-MM-DD')
              } else {
                state.formData.village_start_audit_at = dayjs().format('YYYY-MM-DD')
              }
            }
    ...
    const finalParams = (flag) => {
      let requestData = {
        ...
        build_end_at: state.formData.build_end_at ? dayjs(state.formData.build_end_at).endOf('month').format('YYYY-MM-DD') + ' 23:59:59' : '',
        ...
        village_start_audit_at: dayjs(state.formData.village_start_audit_at).format('YYYY-MM-DD HH:mm:ss'),
    ...
    const onSingleConfirm = (value) => {
      ...
      fieldChange()
      state.formData.village_start_audit_at = str
    }
