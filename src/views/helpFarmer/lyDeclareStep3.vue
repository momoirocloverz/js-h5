  ...
  <div>
    ...
    <div class="help-farmer-container" :class="{ 'fit-old': ZLBFitForOld }">
      <div class="panel">
        <div class="form-conatiner">
          <van-form @submit="onSubmit" @failed="onFailed" :disabled="status > 0" label-width="140" validate-trigger="onSubmit" ref="acForm">
            ...
            <AcUploader :fileList="lzhtFileList" title="1.种植区域农村土地承包经营权委托流转合同" :witchType="72" @deleteCb="getData" @choiceImg="choiceImg" :status="status"></AcUploader>
            <AcUploader :fileList="lzqcFileList" title="2.种植区域耕地承包流转清册" :witchType="73" @deleteCb="getData" @choiceImg="choiceImg" :status="status"></AcUploader>
            <AcUploader :fileList="sytFileList" title="3.种植区域分布示意图（现状图）" :witchType="74" @deleteCb="getData" @choiceImg="choiceImg" :status="status"></AcUploader>
            <AcUploader :fileList="zfpzFileList" title="4.大田租金支付凭证或分户支付清册" :witchType="75" @deleteCb="getData" @choiceImg="choiceImg" :status="status"></AcUploader>
          </van-form>
        </div>
      </div>
      <!-- 审核流程 -->
      ...
      <div class="bottom-btn" v-if="status != 82">
        ...
        <div class="btn" style="width: 33.333%" @click="validateForm">提交</div>
      </div>
