  ...
  <div class="approval-steps">
    ...
    >
      <van-step v-for="(item, key, index) of processData" :key="index">
        ...
        <template v-if="index > 0 && statusTag > 0 && statusTag >= index">
          ...
          <div class="opinion" v-if="statusTag > index && statusTag != 1">
            意见：{{ approvalData[item.reason] || '-' }}
          </div>

          <div class="opinion" v-if="item.real_fund_amount">
            拨付金额：{{ approvalData[item.real_fund_amount] || '-' }}万元
          </div>

          <div
            class="opinion"
            @click="showImages(approvalData[item.attachment])"
            v-if="statusTag > index"
          >
          ...
          <div class="opinion" v-if="statusTag > index && approvalData[item.attachment_doc] && approvalData[item.attachment_doc].length">
            <span
              class="link doc"
              v-for="(doc, i) in approvalData[item.attachment_doc]"
              :key="i"
              @click="showDocs(doc)"
              >{{ `文件附件${i + 1}` }}</span
            >
    ...
    >
      <van-step v-for="(item, key, index) of processData" :key="index">
        ...
        -->
        <div class="opinion" v-if="index > 0 && statusTag - 2 > index">
          意见：{{ approvalData[item.reason] || '-' }}
        </div>
        <!--
          操作时间
        ...
        <div
          class="opinion"
          @click="showImages(approvalData[item.attachment])"
          v-if="index > 0 && statusTag - 2 > index"
        >
          <span class="link">查看附件</span>
        </div>
        <div class="opinion" v-if="index == 2 && status == 42">
          拨付金额：{{ `${approvalData[item.real_fund_amount] || '-'}万元` }}
        </div>
        <template #active-icon> <div class="dot done"></div> </template>
        <template #inactive-icon> <div class="dot"></div> </template>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const generateTag = status => {
      ...
      return parseInt(status.toString().substr(0, 1))
    }
    ...
    const generateTail = status => {
      ...
      return parseInt(status.toString().substr(statusStr.length - 1, 1))
    }
